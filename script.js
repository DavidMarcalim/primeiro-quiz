...
function quizCodigo(container) {
    const perguntas = [
        ...
        { pergunta: "Qual o plural de 'míssil'?", opcoes: [{ texto: "Míssis", correto: false }, { texto: "Mísseis", correto: true }, { texto: "Míssils", correto: false }] },
...
function quizTudo(container) {
    ...
    const perguntasCiencias = quizEmails_perguntas();
    const perguntasPortugues = quizCodigo_perguntas();
    const perguntasBiblia = quizApresentacao_perguntas();
    const perguntasMatematica = quizReuniao_perguntas();
...
function renderizarQuizComVidas(container, perguntas) {
    if (vidas <= 0) {
        mostrarMensagem("Você perdeu todas as suas vidas! Game Over.");
        setTimeout(voltarAoPC, 2000);
        return;
    }

    let perguntaDisponivel = null;

    // Encontrar uma pergunta que ainda não foi feita
    let perguntasNaoFeitas = perguntas.filter(p => !perguntasFeitas.includes(p));

    if (perguntasNaoFeitas.length === 0) {
        mostrarMensagem("Parabéns! Você respondeu a todas as perguntas! Voltando ao PC...");
        adicionarXp(200); // Recompensa extra por completar o quiz
        setTimeout(voltarAoPC, 2000);
        return;
    }
    ...
    container.innerHTML = `
        ...
        <p class="text-lg mb-2">Vidas restantes: <span id="vidas-restantes" class="font-bold text-red-500">${vidas}</span></p>
...
window.verificarRespostaComVidas = function(perguntaIndex, indiceOpcao) {
    ...
    if (respostaCorreta) {
        mostrarMensagem("Parabéns! Resposta correta! Você ganhou 50 XP!");
        xp += 50;
        perguntasFeitas.push(quizPerguntas[perguntaIndex]); // Adiciona a pergunta ao histórico
    } else {
        vidas--;
        mostrarMensagem(`Oops, resposta incorreta! Você perdeu 1 vida. Vidas restantes: ${vidas}`);
    ...
    setTimeout(() => {
        ...
        // A lógica de promoção já está no adicionarXp, então não precisamos chamar atualizarStatus aqui.
        // Apenas recarregamos o quiz.
        const todasPerguntas = [
